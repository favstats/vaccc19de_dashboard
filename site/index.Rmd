...
tweet_dat <- latest_dat %>% 
  ...
  mutate(tweet_raw2 = glue::glue("{bundesland_iso}: {impfungen_kumulativ}")) %>% 
  ...
  tweet3 <- tweet_dat %>% 
    ...
    pull(tweet3) %>% 
    paste0(glue::glue("Differenz zum Vortag ({current_day}):\n\n"), .)
  
  tweet4 <- tweet_dat %>% 
    summarise(tweet4 = paste0(tweet_raw4, collapse = "\n")) %>% 
    pull(tweet4) %>% 
    paste0(glue::glue("Prozent Wachstum zum Vortag ({current_day}):\n\n"), .)

  
  
  post_tweet(status = tweet1, media = tempfile1)
  
  Sys.sleep(10)
  
  post_tweet(status = tweet2, media = tempfile1)
  
  Sys.sleep(10)
  
  post_tweet(status = tweet3, media = tempfile3)
  
  Sys.sleep(10)
  
  post_tweet(status = tweet4, media = tempfile4)
}
