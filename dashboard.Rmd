...
rki_dat %>% 
  ...
  hc_colors(colors =  colorspace::qualitative_hcl(16, palette = "Dynamic", rev = T)
  )
...
rki_dat %>% 
  ...
  # Axis
  hc_yAxis(
    title = list(text = "Anzahl der Impfungen (kumulativ)")
  ) %>% 
  hc_xAxis(
    title = list(text = "Datum (täglich)")
    )  %>% 
...
### Anzahl der Anzahl der Impfungen {.value-box}
